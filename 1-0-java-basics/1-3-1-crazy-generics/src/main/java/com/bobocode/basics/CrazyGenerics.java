...
public class CrazyGenerics {
    ...
    static class CollectionUtil{
        ...
        public static <T extends BaseEntity> boolean hasDuplicates(Collection<T> entities, T targetEntity) {
            long count = entities.stream().parallel().filter(element -> element.getUuid()
                .equals(targetEntity.getUuid())).count();
            return count > 1;
        }
        ...
        public static <T> Optional<T> findMax(Iterable<T> elements, Comparator<? super T> comparator){
            ...
            while(iterator.hasNext() == true){
                var curVal = iterator.next();
                if (comparator.compare(curVal, maxVal)>0) {
                    maxVal = curVal;
                }
            }
        ...
        public static <T> void answer(List<T> elements, int i, int j) {
